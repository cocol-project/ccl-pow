...
describe BTCPoW::Utils do
  ...
  it "finds new target" do
    new_target = BTCPoW::Utils.retarget(
      start_time: (Time.utc_now.to_unix - 12).to_f64, # we test for 12 seconds
      end_time: Time.utc_now.to_unix.to_f64,
  ...
  #   pp "-- Start: #{start_time}"
  #   (1..30).each do |i|
  #     block = BTCPoW.mine(difficulty: diff, for: "TestMe#{Time.now}#{Random::Secure.hex}")
  ...
  #   retarget = BTCPoW::Utils.retarget(
  #     start_time: start_time.to_unix,
  #     end_time: end_time.to_unix,
  #     wanted_timespan: 30_u32,
  #     current_target: BTCPoW::Utils.calculate_target(diff)
  #   )
